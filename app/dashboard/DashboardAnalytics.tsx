        ...
        <div className="space-y-6">
            ...
            <div className="flex bg-zinc-100 dark:bg-zinc-800/50 p-1 rounded-xl w-fit">
                {(['today', 'week', 'month'] as TimePeriod[]).map(p => (
                    ...
                    >
                        {p === 'today' ? t('dashboard.period_today') : t('dashboard.period_this').replace('{period}', p)}
                    </button>
                ))}
            </div>
            ...
            {/* Revenue Chart */}
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/60 rounded-[1.5rem] p-8 shadow-sm">
                 <h3 className="text-lg font-bold mb-6">
                    {period === 'today' ? t('dashboard.chart_hourly_today') : period === 'week' ? t('dashboard.chart_last_7_days') : t('dashboard.chart_daily_month')}
                 </h3>
                 <div className="h-64 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={charts[period]} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                            ...
                            <YAxis axisLine={false} tickLine={false} tick={{fill: '#a1a1aa', fontSize: 12}} tickFormatter={(value) => formatCurrency(value, (stats as any).currency)} />
                            <Tooltip 
                                cursor={{fill: '#f4f4f5'}}
                                contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', backgroundColor: '#18181b', color: '#fff'}}
                                itemStyle={{ color: '#fff' }}
                                formatter={(value: any) => [formatCurrency(value, (stats as any).currency), t('dashboard.tooltip_revenue')]}
                            />
