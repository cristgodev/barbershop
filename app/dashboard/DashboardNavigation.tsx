...
export default function DashboardNavigation({ shop, userRole }: { shop: any, userRole?: string }) {
    const pathname = usePathname()
    const { t } = useTranslation()
    const { isOpen, setIsOpen } = useMobileNav()

    const isOwnerOrSuper = userRole === 'OWNER' || userRole === 'SUPERADMIN'
    const isStaffFull = userRole === 'OWNER' || userRole === 'SUPERADMIN' || userRole === 'MANAGER' || userRole === 'ADMIN'
    ...
    const navItems = [
        ...
        isStaffFull && shop.isPosEnabled && { name: 'Inventario', href: '/dashboard/inventory', icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
        isStaffFull && shop.isMarketingEnabled && { name: 'Marketing', href: '/dashboard/marketing', icon: 'M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z' },
        isStaffFull && shop.isLoyaltyEnabled && { name: 'Loyalty (Rewards)', href: '/dashboard/loyalty', icon: 'M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7' },
        isOwnerOrSuper && { name: t('dashboard.menu_staff'), href: '/dashboard/staff', icon: 'M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14' },
        isOwnerOrSuper && { name: t('dashboard.menu_services'), href: '/dashboard/services', icon: 'M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5' },
        ...
        userRole === 'SUPERADMIN' && { name: 'SuperAdmin Panel', href: '/superadmin', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' }
        ...
        <>
            ...
            <aside className={`w-64 bg-white dark:bg-zinc-950 border-r border-zinc-200 dark:border-zinc-800 flex flex-col fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="h-16 flex items-center px-6 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shrink-0 justify-between">
                    ...
                    <button 
                        onClick={() => setIsOpen(false)}
                        className="md:hidden p-2 -mr-2 text-zinc-500 hover:text-black dark:hover:text-white"
                    >
