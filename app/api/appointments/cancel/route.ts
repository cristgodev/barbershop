...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Verify permissions - only the barber assigned, the owner, or the customer can cancel
        const isAssignedBarber = session.user.id === appointment.barberId
        const isCustomer = session.user.id === appointment.customerId
        // Check if the user is an OWNER of this specific barbershop
        const isShopOwner = session.user.role === 'OWNER' && session.user.barbershopId === appointment.barbershopId

        if (!isAssignedBarber && !isCustomer && !isShopOwner) {
